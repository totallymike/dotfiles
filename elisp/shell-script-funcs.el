...
(defun mw/substring-replace (old-str new-str beg end)
  ...
  (if (string-prefix-p "~/" str)
    ...
    (let ((s (or (string-match "${\\([^ }]*\\)}" str)
      ...
      (if (not s)             ; No $ matches?
        ...
        (mw/substr-variables  ; Recursively call with first var sub'd
         (mw/substring-replace str (mw/getvar (match-string 1 str)) s e))))))

(defun mw/get-files (path &optional full)
  ...
  (let ((subbed-path (mw/substr-variables path)))
    (condition-case nil
...
(defun mw/has-org-tag-p (file tag)
  (let ((reg (concat "^#\\+TAGS:.* " tag "\\b")))
    (with-temp-buffer
      (insert-file-contents file)
      (re-search-forward reg nil t 1))))

(defun mw/get-org-files (path tag &optional full)
...
If FULL is specified, return absolute pathnames for each file."
  (filter (lambda (file) (mw/has-org-tag-p file tag)) (mw/get-files path full)))

;; (ha/get-org-files "~/website/Presentations/*.org" "presentation" t)

(defun mw/get-path (path &rest extra)
  ...
  (let ((file-parts (cons (mw/substr-variables path) extra)))
...
(defun mw/mkdir (path)
  ...
  (make-directory (mw/get-path path) t))


(defun mw/mksymlink (orig link)
  ...
  (let ((orig-file (mw/get-path orig))
        (link-file (mw/get-path link)))

    (if (not (file-symlink-p link-file))
        (make-symbolic-link orig-file link-file t))))

(defun mw/mksymlinks (files dest)
  ...
  (mapc (lambda (file) (mw/mksymlink file dest)) (mw/get-files files t)))


(defun mw/mkdirs (path)
  ...
  (mapc (lambda (dir) (make-directory dir t)) (mw/get-files path)))

(defun mw/copy-files (from to files)
  "Copy some files FROM a directory TO another directory, where FILES is a list of names."
  (mapcar (lambda (file)
            (copy-file (mw/get-path from file)
                       (mw/get-path to) t))     files))
